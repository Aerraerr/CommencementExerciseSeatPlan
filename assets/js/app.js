...
// ================= EXECUTIVE PRIORITY (COURSE BASED) =================
function prioritizeExecutives(students) {
    let executives = [];
    let others = [];

    students.forEach(s => {
        if ((s.Course || "").toLowerCase() === "executive") {
            executives.push({
                ...s,
                isExecutive: true
            });
...
function distributeStudents(students) {
    ...
    // ================= EXECUTIVES FIRST (Cluster B priority) =================
...
function render(clusters) {
    ...
    ["A", "B", "C", "D"].forEach(key => {
        ...
        clusters[key].forEach(student => {
            ...
            // ================= EXECUTIVE COLOR (LIGHT BLUE) =================
            if (student.isExecutive || (student.Course || "").toLowerCase() === "executive") {
...
function buildList(clusters) {
    ...
    if (currentSort === "college") {
        ...
        allStudents.forEach(s => {
            let college = s.College || "N/A";
            if (!collegeOrder.includes(college)) collegeOrder.push(college);
        });

        collegeOrder.forEach(college => {
            ...
            allStudents
                .filter(s => (s.College || "N/A") === college)
                .forEach(s => {
